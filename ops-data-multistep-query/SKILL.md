...
**8.3 提取结果**：结果文本出现在 `分析完成` 之后的 StaticText 行（常含一段总结句 + 可选表格）。

> ⚠️ 不要用 `evaluate_script` 抓 `document.querySelectorAll('table')` —— AI助手页面表格在 Shadow DOM 中渲染，`querySelectorAll('table')` 返回 0 个结果。应从快照文本解析回答内容：找到 `分析完成` 之后、反馈区域（`您的反馈对我们非常重要`）之前的文本区域，提取表格行列数据和 AI 总结文本。

**分页处理**：表格数据超过 5 行时会分页显示（如 `1/2`、`2/2`）。翻页方法：

```bash
# 1. 点击分页指示器（如 "1/2" 的 generic 元素）打开下拉列表
chrome-devtools click "<分页指示器 uid>"
chrome-devtools take_snapshot

# 2. 从下拉列表中点击目标页码（如 "2/2" 的 StaticText）
chrome-devtools click "<目标页码 uid>"
sleep 1
chrome-devtools take_snapshot
...
重复翻页直到所有页面数据提取完毕。将所有页的数据合并后，再从中提取**下一轮所需的变量**（如宿主机名称），回填到 `Q(i+1)` 模板。
...
8. **表格在 Shadow DOM 中**：不要用 `evaluate_script` + `querySelectorAll('table')` 提取表格，应从快照文本解析。
9. **分页翻页**：表格结果超过 5 行会分页，点击分页指示器（如 `1/2`）打开下拉，选择目标页码翻页，直到所有页面数据提取完毕。
